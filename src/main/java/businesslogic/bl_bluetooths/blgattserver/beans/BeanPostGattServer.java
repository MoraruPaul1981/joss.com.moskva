...
import businesslogic.bl_bluetooths.blgattserver.binesslogic.BinesslogicPostGattServer;
...
@Stateless(mappedName = "BeanPostGattServer")
@LocalBean
@Transactional
@TransactionAttribute(TransactionAttributeType.REQUIRED)
public class BeanPostGattServer {
    ...
    @Inject
    private BinesslogicPostGattServer binesslogicPostGattServer;
    ...
    @Asynchronous
    public void staringPostGattServer(@NotNull ServletContext ЛОГ,
        ...
        try {
            ...
            if (sessionGattServer.isOpen() && sessionGattServer.getTransaction().getStatus()==TransactionStatus.ACTIVE) {
                ...
                String    JobForServerScaner = Optional.ofNullable(request.getParameter("JobForServer")).map(String::new).orElseGet(()->"");

                if(!JobForServerScaner.isEmpty()){

                    switch (JobForServerScaner.trim()) {
                        case "sendgattserver":

                            // TODO: 07.09.2024 Worker JAkson JSON Ot client GATT
                            byte[]    БуферРезультатPOSTGattServer = binesslogicPostGattServer.proccestingbinesslogicGattServer(ЛОГ, request, response, sessionGattServer );

                            // TODO: 07.09.2024 Otvet Client Success
                            bEANCallsBack.МетодBackДанныеКлиентуByte(response, БуферРезультатPOSTGattServer, ЛОГ  );
                            ...
                            ЛОГ.log( " Класс"+Thread.currentThread().getStackTrace()[2].getClassName()
                                    ...
                                    + " БуферРезультатPOSTGattServer " +БуферРезультатPOSTGattServer);
                            break;
                    }
                }
            }
