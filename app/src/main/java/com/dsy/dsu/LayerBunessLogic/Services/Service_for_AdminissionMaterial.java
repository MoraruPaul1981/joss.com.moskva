...
import com.dsy.dsu.LayerBunessLogic.DATE.SubClassMONTHONLY_ТолькоАнализ;
import com.dsy.dsu.LayerBunessLogic.DATE.SubClassYEARONLY;
import com.dsy.dsu.LayerBunessLogic.DATE.SubClassYearHONLY_ТолькоАнализ;
import com.dsy.dsu.LayerBunessLogic.CnangeServers.PUBLIC_CONTENT;
import com.dsy.dsu.LayerBunessLogic.DATE.SubClassMONTHONLY;
    ...
    class SibClassApplyFromBackPeriodof_ЗаполененияТабеляИзПрошлогоМесяца {
        ...
        private void МетодЗапускЗаполенеияИзПрошлыхМесяцев(@NonNull Context context, @NonNull Intent intent) {
            try {
                ...
                Integer ПубличныйIDДляЗаполененияИзПрошлогоМесяца = new Class_Generations_PUBLIC_CURRENT_ID(context).getPublicIDAllApp(getApplicationContext());
        ...
        private Integer МетодВставкивТаблицуДата_Табель(@NonNull Context context,
            ...
            try {
                ...
                Long РезультатУвеличиваемВерсияДатаТАбель =
                        new SubClassUpVersionDATA(context).upVersionCurentTable(НазваниеОбрабоатываемойТаблицы,context);
            ...
            try{
                asyncTaskLoader=new AsyncTaskLoader<Integer>(context) {
                    ...
                    public Integer loadInBackground() {
                        ...
                        Integer  ПубличныйIDДляФрагмента = new Class_Generations_PUBLIC_CURRENT_ID(getApplicationContext()).getPublicIDAllApp(context);
                        ...
                        Long РезультатУвеличиваемВерсияПолучениеНовогоМатериала =
                                new SubClassUpVersionDATA(getApplicationContext()).upVersionCurentTable(НазваниеОбрабоатываемойТаблицы,getApplicationContext());
                        ...
                        public void accept(LinkedHashMap<String, Bitmap> stringBitmapLinkedHashMap) throws Throwable {
                            try{
                                ...
                                if (ImageРАзмерФотографии>0) {
                                    ...
                                    // TODO: 18.03.2023  получаем ВЕСИЮ ДАННЫХ
                                    Long РезультатУвеличиваемВерсияПолучениеНовогоМатериала =
                                            new SubClassUpVersionDATA(getApplicationContext())
