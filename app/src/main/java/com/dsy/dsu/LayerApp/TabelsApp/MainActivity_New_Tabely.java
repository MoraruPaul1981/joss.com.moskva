...
import com.dsy.dsu.LayerBunessLogic.Services.Service_for_AdminissionMaterial;
...
while(iterator.hasNext()){
        ...
        try {
         ...
         Long   СгенерированныйUUIDДляНовогоТабеля=  (Long)   new Class_Generation_UUID(getApplicationContext()).МетодГенерацииUUID();
            ...
            АдаптерВставкиНовгоТабеля.put("date_update",СгенерированованныйДатаДляДаннойОперации);
                ...
                Long getPublicID=  new Class_Generations_PUBLIC_CURRENT_ID(getApplicationContext()).gettingSettingTableVersion(getApplicationContext()," SELECT publicid FROM successlogin "  ,"successlogin");
                // TODO: 14.12.2022  
                ...
                Long РезультатУвеличинаяВерсияДАныхЧата =
                        new SubClassUpVersionDATA(getApplicationContext()).upVersionCurentTable(   "tabel",getApplicationContext() );
    ...
    protected   Cursor МетодДляНовогоТабеляПолучаемДанные(@NonNull String  ФлагКакаяТаблицаОбработки){
        Cursor cursor = null;
        try{
         Integer   ПубличныйIDДляФрагмента     = new Class_Generations_PUBLIC_CURRENT_ID(getApplicationContext()).
