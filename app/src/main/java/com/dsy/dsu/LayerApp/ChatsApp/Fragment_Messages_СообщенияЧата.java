package com.dsy.dsu.LayerApp.ChatsApp;
                               ...
                               Log.d(this.getClass().getName(), " ClassActitytyClassActityty  view.getId() " + view.getId()+ " columnIndex "+ columnIndex  + " cursor " +cursor);
                                   ...
                                   // TODO: 28.06.2021  главное ДЕЙСТВИЕ ЗАПОЛЯНЕМ ДАННЫМИ НАШ SIMPLECURSORADAPTER
                                       ...
                                       МетодГлавныйЗаполняемДаннымиНашSimpleCursorAdapter((TextView) view,
                                            cursor,subClassGET_fio,getPublicID
     ...
     do{
                 ...
                 Log.d(this.getClass().getName(), "чтовкурсоре "  + чтовкурсоре);
                           ...
                           // Class_GRUD_SQL_Operations.GetData class_grud_sql_operationsФрагментМСообщения= class_grud_sql_operationsCтатусХотьестьОДинНольНЕПрочттаноеСообщениевЧате.new GetData(getContext());
                            ...
                            if (getPublicID.compareTo(IDдляпервогоФрагментаПользовательКоторыйНаписал) !=0) {
                                ...
                                МетодВычисляемЕслиХотьОдноСообщениеНеПрочитаноДляТекущегоПользоватеялСтрочки(UUIDдляпервогоФрагментаДляСообщенияTagДляПередачиВоДругиеФрагменты,getPublicID
                                        , public_contentCompletionService);
...
*/
                ...
                if (Курсор_ВычисляемПУбличныйID.getCount() > 0) {
                    ...
                    //////////////
                    getPublicID = Курсор_ВычисляемПУбличныйID.getInt(0);
                    //////
                    Log.d(this.getClass().getName(), "ID " + getPublicID);
                ...
                Log.d(this.getClass().getName(), "  MODEL  ID " + getPublicID);
...
//               /// class_grud_sql_operationsMODEL. concurrentHashMapНабор.put("УсловиеЛимита","1");
                ...
                Integer ПубличныйIDДляФрагмента = new Class_Generations_PUBLIC_CURRENT_ID(getContext()).getPublicIDAllApp(getContext());
